...
export async function GET(_req: Request, { params }: { params: Promise<{ memberId: string }> }) {
  ...
  if (!memberId) return Response.json({ hasProfile: false, hasApprovedProfile: false, profileId: null });
  ...
  return Response.json({ hasProfile: !!p, hasApprovedProfile: approved, profileId: p?.id ?? null });
}
