...
export interface ProfileEditorProps {
  /** The profile to edit, already loaded by the caller. */
  initial: OwnProfileDetail;
  /** Persist the edit. The caller chooses the owner or admin endpoint. */
  onSave: (input: UpdateProfileInput) => Promise<void>;
  /** Called after a successful save (e.g. navigate away). */
  onSaved: () => void;
  /** Where the Cancel button goes. */
  cancelHref: string;
  /** Show the "editing re-enters verification" note (owner edits only). */
  reverifyNote?: boolean;
}

/** Shared registration-style editor used by both the owner edit page and the admin edit page. */
export function ProfileEditor({ initial, onSave, onSaved, cancelHref, reverifyNote = false }: ProfileEditorProps) {
  const { t } = useT();
